...
final class ExerciseViewModel: ObservableObject {
    ...
    // MARK: - Open actions
    /// ✅ Premium限定の入口（無料なら false を返す）
    @discardableResult
    func openSheet(_ sheet: ExerciseSheet, isPremium: Bool) -> Bool {
        gateMessage = nil

        guard isPremium else {
            gateMessage = "この資料はプレミアム会員で閲覧できます。"
            return false
        }

        guard let url = URL(string: sheet.url) else {
            gateMessage = "URLが不正です。"
            return false
        }
